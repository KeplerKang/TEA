...
def load_new(net, checkpoint, partial_restore=False, teaorstu = 'teacher', frozen=False, strict = False, device=None):  
    assert checkpoint is not None, "no path provided for checkpoint, value is None"
    if os.path.isdir(checkpoint):
        checkpoint = max(glob.iglob(checkpoint + '/*.pth'), key=os.path.getctime)
        print("loading model from %s" % checkpoint)
    ...
    if partial_restore:
        if teaorstu == 'teacher':
            pre_name = 'teacher_net.'
        elif teaorstu == 'student':
            pre_name = 'student_net.'
        net_dict = net.state_dict()
        load_dict = {}
        for k, v in net_dict.items():
            if k in saved_net:
                load_dict[k]=saved_net[k]
            elif k=='student_net.class_prototypes':
                load_dict[k]=saved_net['teacher_net.class_prototypes']
            else:
                load_dict[k]=v
        # print("params to keep from checkpoint:")
        # print(load_dict.keys())
        # extra_params = {k: v for k, v in net_dict.items() if k not in load_dict}
        # print("params to randomly init:")
        # print(extra_params.keys())
        # for param in extra_params:
        #     saved_net[param] = net_dict[param]
    else:load_dict = saved_net

    miss, _ = net.load_state_dict(load_dict, strict=strict)
    train_params = []
    if frozen:
        for name, param in net.named_parameters():
            if name in saved_net or name == 'student_net.class_prototypes':
                param.requires_grad = False
            else:
                train_params.append(name)
    # print(f"successfully load from checkpoint {checkpoint}, \ntrainable params: {train_params},\nmiss:{miss},\nfrozens:{frozens}")
    return checkpoint, train_params
