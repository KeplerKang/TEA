...
class FeatureMemory:
    def __init__(self, memory_per_class=10000, n_classes=18, feat_dim=128, proj_dim=128, k=[8,16,32]):
    ...
    def update(self, fts, rep, labels):
        fts_dim, rep_dim = fts.size(2), rep.size(2)
        
        fts = fts.permute(0,1,3,4,2) # B K H W C
        rep = rep.permute(0,1,3,4,2) # B K H W C
        
        data = torch.cat([fts, rep], dim=-1)
        data = gather_together(data)

        fts = data[:,:,:,:,:fts_dim] # B K H W C
        rep = data[:,:,:,:,fts_dim:fts_dim+rep_dim] # B K H W C
        labels = gather_together(labels) # B H W
        ...
        for c in range(self.n_classes):
            ...
            fts_c = fts_c[mask_c, :].clone()# get fts from class c
            rep_c = rep_c[mask_c, :].clone()
...
def compute_dgcl_loss(
    ...
    for cls in batch_cls:
        ...
        fts_c = fts[:,cls-1].clone()
        rep_c = rep[:,cls-1].clone()

        feat_cls = fts_c[cls_map] # N 256
        proj_cls = rep_c[cls_map] # N 256
        ...
        feat_memo_cls = feat_memory[cls-1] # 5000 256
        ...
        memo_cls_density = in_memo_density[cls-1] # memo_size
        _, idx_pos = torch.topk(memo_cls_density, k=k_high_thresh)

        proj_pos_m = proj_memory[cls-1][idx_pos] # 50 256
        ...
        proj_neg = rep_c[not_cls_map].detach().clone()# (num_cls-1)*50 256
