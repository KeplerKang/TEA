...
def train_and_evaluate(net, dataloaders, config, device, save_path, lin_cls=False):
    ...
    def train_step(net, sample, loss_fn, optimizer, device, loss_input_fn):
        ...
        loss = loss_teacher + loss_student + all_mse_loss

        loss.backward()
        optimizer.step()
        return teacher_x, student_x, ground_truth, loss, loss_teacher, loss_student, all_mse_loss
    ...
    checkpoint, train_params = load_new(net, '/workspace/S/lijin/kang/my_codes/ours/exps/0820_mix_tea/best.pth', partial_restore=True,teaorstu='teacher',frozen=True,device=device)
    callog(f"successfully load from checkpoint {checkpoint}")
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):  # loop over the dataset multiple times
        for step, sample in enumerate(dataloaders['train']):
            ...
            teacher_x, student_x, ground_truth, loss, loss_teacher, loss_student, mse_loss = train_step(net, sample, loss_fn, optimizer, device, loss_input_fn=loss_input_fn)
            ...
            if abs_step % train_metrics_steps == 0:
                ...
                callog("abs_step: %d, epoch: %d, step: %5d, total_loss: %.7f, loss_teacher: %.7f, loss_student: %.7f, mse_loss: %.7f, batch_teacher_iou: %.4f, "
