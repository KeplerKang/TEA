...
def train_and_evaluate(net, dataloaders, config, device, lin_cls=False):
    ...
    def train_step(net, sample, loss_fn, optimizer, device, loss_input_fn):
        ...
        input = sample['inputs']
        outputs,loss1= net(input.to(device))
        outputs = outputs.permute(0, 2, 3, 1)
        ground_truth = loss_input_fn(sample, device)
        loss2 = loss_fn['mean'](outputs, ground_truth)
        loss = loss1.mean()+loss2
        loss.backward()
        optimizer.step()
        return outputs, ground_truth, loss
  
    def evaluate_test(net, evalloader, loss_fn, config,mode='eval'):
        net.eval()
        ioussss = []
        ...
        for ratio in ratios:
            ...
            callog("-------------------------------------------------------------------------------------")
        callog(f"AUC:{str(ioussss[-1])}, mean_AUC:{str(sum(ioussss)/len(ioussss))}")
    ...
    test_metrics = evaluate_test(net, dataloaders['test'], loss_fn, config,mode='test')
